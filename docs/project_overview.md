...
- Архитектура в `drawio`: [relocation_agent_architecture.drawio](relocation_agent_architecture.drawio)
- Функциональный flow в `drawio`: [relocation_agent_functional_flow.drawio](relocation_agent_functional_flow.drawio)
- Production contour в `drawio`: [relocation_agent_production_contour.drawio](relocation_agent_production_contour.drawio)
- Архитектура в `PlantUML`: [relocation_agent_architecture.puml](relocation_agent_architecture.puml)
- Разбор проекта по критериям и список оставшихся gap'ов: [project_criteria_gap_analysis.md](project_criteria_gap_analysis.md)
- Operating notes по `data freshness`, `auth`, `observability`: [production_operating_model.md](production_operating_model.md)
- Готовые demo-сценарии со скриншотами: [demo_scenarios.md](demo_scenarios.md)
- Framework оценки агента на live LLM + human feedback rubric: [evaluation_framework.md](evaluation_framework.md)
- Реальный внешний MCP provider и пример shortlist: [external_mcp_provider_example.md](external_mcp_provider_example.md)
...
- `scraper/` — самописный сбор объявлений с `Krisha` и `List.am`.
...
- baseline и сравнение вариантов описаны в [evaluation_framework.md](evaluation_framework.md)
...
- `Streamlit`-интерфейс с чатом, sidebar по кейсам, трассировкой графа и shortlist;
...
- runbook и prompt-ы: [demo_scenarios.md](demo_scenarios.md)
...
- [`src/app/assets/workflow_overview.svg`](../src/app/assets/workflow_overview.svg) — картинка общего пайплайна;
- [relocation_agent_functional_flow.drawio](relocation_agent_functional_flow.drawio) — схема сценария "запрос -> shortlist -> ответ";
- [relocation_agent_architecture.drawio](relocation_agent_architecture.drawio) — верхнеуровневая архитектура;
- [relocation_agent_production_contour.drawio](relocation_agent_production_contour.drawio) — target production contour;
...
- [external_mcp_provider_example.md](external_mcp_provider_example.md)
...
- реальные market data шумные и быстро устаревают, поэтому потребовался отдельный слой нормализации;
...
- полный dialog graph зависит от structured LLM для `router`, `intake`, `replanner` и `final_composer`;
...
2. Поднять actual quality по router, escalation и narrative-метрикам до прохождения quality gate.
